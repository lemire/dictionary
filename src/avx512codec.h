#ifndef DICT_AVX512_H
#define DICT_AVX512_H
...
#include "avx512bpacking.h"
#include "avx512dict.h"
#include "dict.h"
...
public:
    ...
    *
    * For simplicity, array lengths are assumed to be multiples of 512.
    */
    inline dictionary_coded_t compress(const uint64_t * array, size_t length) {
        ...
        avx512packwithoutmask(tmpbuffer,(__m512i *) out.compressed_data, out.array_length, out.bit_width);
        return out;
    }
    ...
    */
    inline uint32_t uncompress(const dictionary_coded_t & t, uint64_t * out) {
        ...
        assert(t.array_length % 512 == 0);
        assert(t.array_length * t.bit_width ==  t.compressed_data_size * 8);
        avx512unpack((const __m512i*) t.compressed_data, tmpbuffer, t.array_length, t.bit_width);
    ...
    */
    static inline uint32_t fastuncompress(const dictionary_coded_t & t, uint64_t * out) {
        assert(t.array_length % 512 == 0);
        assert(t.array_length * t.bit_width ==  t.compressed_data_size * 8);
        avx512unpackdict((const __m512i*) t.compressed_data,
    ...
    *
    * For simplicity, all indexes and lengths are are assumed to be multiples of 512.
    ...
    */
    static inline uint32_t fastrangeuncompress(const dictionary_coded_t & t, uint64_t * out, size_t start, size_t length) {
        ...
        avx512unpackdict((const __m512i*) ( t.compressed_data + start * t.bit_width / 8),
...
private:
    ...
    AVX512DictCODEC(const AVX512DictCODEC & ) : tmpbuffer(NULL), buffercapacity(0) {}


    AVX512DictCODEC& operator=(const AVX512DictCODEC & ) {
